const { stringToInteger, stringToDate, toTimeString } = require('../lib/utils');
...
const update = async (db, textMessages, userId = null, order) => {
  try {
    ...
    orderFields['wait_minutes'] && (orderFields['fulfilled_at_est'] = new Date(Date.now() + safeOrder.waitMinutes * 60 * 1000));
